...
(function(){
  ...
  function connect(){
    ...
    ws.onclose = (e)=>{
      ...
      reconnectAttempts++;
      const delay = Math.min(20000, 5000 + reconnectAttempts * 3000); // CHANGE: Exponential backoff
...
app.get('/logs', (req, res) => {
  ...
  res.setHeader('Keep-Alive', 'timeout=30'); // CHANGE: Add keep-alive
  res.flushHeaders?.();
  const send = (line) => res.write(`data: ${line}\n\n`);
  const onLine = (line) => send(line);
  bus.on('line', onLine);
  send(`${new Date().toISOString()} - [SSE] connected from ${req.ip || req.socket.remoteAddress}`);
  const iv = setInterval(() => send(`${new Date().toISOString()} - [SSE] keepalive`), 10000); // CHANGE: Reduced to 10s
...
function attachTunnelWSS(server) {
  const wss = new WebSocketServer({
    server,
    path: '/ws-tunnel',
    perMessageDeflate: false, // CHANGE: Keep disabled for Heroku compatibility
    maxPayload: 1024 * 1024
  });

  // CHANGE: Restore custom upgrade handler from working version
  server.on('upgrade', (req, socket, head) => {
  ...
  wss.on('connection', (ws, req) => {
    ...
    try {
      ...
      setTimeout(() => {
        if (ws.readyState === ws.OPEN) {
          safeSend(ws, { type: 'ping', id: 'server-init', serverTs: Date.now() });
    ...
    const appBeat = setInterval(() => {
      ...
      log('TUNNEL sent heartbeat ping'); // CHANGE: Log heartbeat
    }, 3000); // CHANGE: Match working version’s 3s
...
function safeSend(ws, obj) { 
  try { 
    if (ws.readyState === ws.OPEN) {
      ...
      log('safeSend success: '+JSON.stringify(obj).slice(0, 200)); // CHANGE: Log send success
    }
  } catch (e) { 
